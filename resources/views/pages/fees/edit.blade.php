<div class="modal fade" id="edit-{{$fee->id}}-Modal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" style="font-family: 'Cairo', sans-serif;" id="editModalLabel">{{__('trans.edit_fee')}}</h5>
            ...
            <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name_ar" value="{{$fee->getTranslation('name', 'ar')}}" class="form-control border" id="name_ar" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="name_en" class="mr-sm-2">{{__('trans.fee_name_english')}}</label>
                            <input type="text" name="name_en" value="{{$fee->getTranslation('name', 'en')}}" class="form-control border" id="name_en" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <select class="custom-select mr-sm-2" required name="stage_id" id="stage_id">
                                    <option selected value="{{$fee->stage_id}}">{{$fee->stage->name}}</option>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <select class="custom-select mr-sm-2" name="class_id" id="class_id">
                                    <option selected value="{{$fee->class_id}}">{{$fee->class->name}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <select class="custom-select mr-sm-2" required name="academic_year" id="academic_year">
                                    ...
                                    @for($year=$current_year; $year<=$current_year +1 ;$year++)
                                        <option {{$year == $fee->academic_year  ? "selected" : ""}} value="{{ $year}}">{{ $year }}</option>
                                    @endfor
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="cost" value="{{$fee->cost}}" class="form-control border" id="cost" required>
                        </div>
                        <div class="col-12">
                            <label for="notes" class="mr-sm-2">{{__('trans.notes')}}</label>
                            <textarea name="notes" class="form-control border" id="notes" rows="3">{{$fee->notes}}</textarea>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secnodary" data-dismiss="modal">{{__('trans.close')}}</button>
                    <button type="submit" class="btn btn-success">{{__('trans.edit_fee')}}</button>
                </div>
            </form>
        </div>
    </div>
