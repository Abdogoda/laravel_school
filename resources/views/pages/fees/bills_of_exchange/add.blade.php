...
@section('title')
{{__('trans.add_new_bill_of_exchange')}}
@stop
@endsection
@section('page-header')
<!-- breadcrumb -->
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0">{{__('trans.add_new_bill_of_exchange')}} @isset ($student_info) ( <a href="{{route('students.show', $student_info->id)}}">{{$student_info->name}}</a> ) @endisset</h4>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                <li class="breadcrumb-item"><a href="{{url('/dashboard')}}" class="default-color">{{__('trans.dashboard')}}</a></li>
                <li class="breadcrumb-item"><a href="{{route('bills_of_exchange.index')}}" class="default-color">{{__('trans.bills_of_exchange_list')}}</a></li>
                <li class="breadcrumb-item active">{{__('trans.add_new_bill_of_exchange')}}</li>
            </ol>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-md-12 mb-30">
        ...
        <div class="card card-statistics h-100">
            <div class="card-body">
                <a href='{{route('bills_of_exchange.index')}}' class="button x-small">{{__('trans.bills_of_exchange_list')}}</a>
                <br><br>
                <form class="row" action="{{ route('bills_of_exchange.store') }}" method="POST">
                    @csrf
                    <div class="card-body">
                        <div class="row">
                        ...
                        <div class="row mt-20 mb-20">
                            <div class="col-12">
                                <button type="submit" class="button">{{ trans('trans.add_new_bill_of_exchange') }}</button>
                            </div>
                        </div>
                    </div>
                </form>
